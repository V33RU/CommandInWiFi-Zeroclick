...
It is useful for IoT researchers, bug hunters working unaudited firmware, educators, and anyone testing their own gear. It is probably not useful as a general pentest tool. Modern named-brand firmware mostly uses library APIs (`nmcli`, NetworkManager D-Bus, libiw) where SSID is passed as a parameter, so the SSID-to-shell class is rare. Beacon-deliverable precedents exist (CVE-2023-45208 D-Link, CVE-2017-2915 / CVE-2017-12094 Circle, CVE-2023-42810 systeminformation, CVE-2021-30800 iOS WiFiDemon), and they cluster in specific segments: cheap consumer routers, CGI-script WiFi configs, older or custom firmware. On most targets the tool will flag nothing, which is the expected outcome.
